...
public class ExObject_01 {
	...
	String name;  //>다른 클래스에서 생성을 거치지 않으면 호출 불가능 // 인스턴스변수.. 일반 인스턴스 멤버변수는 객체생성 후 각각 다른 값을 가질 수 있다
	static final String MESSAGE="Happy Day!!!";
	//static 멤버변수는 주로 상수로 선언할때 사용하며 인스턴스 변수없이 클래스명으로 접근 가능하다 >> 생성을 하지 않아도 호출 가능
